//! Provisioning runtime — playbook execution, tag catalog resolution, and role config discovery.
//!
//! Testing strategy:
//! The argument formatting and binary resolution logic in `AnsibleRuntime` is extracted into
...
/// Runtime implementation for provisioning contracts.
pub struct AnsibleRuntime {
    ansible_dir: PathBuf,
...
impl AnsibleRuntime {
    ...
    /// Empty runtime for contexts that do not require provisioning assets.
    ...
    pub(crate) fn build_command_with_env<T: AsRef<std::ffi::OsStr>>(
        ...
        if self.ansible_dir.as_os_str().is_empty() {
            return Err(AppError::AnsibleExecution {
                message: "provisioning runtime not initialised (no ansible_dir)".to_string(),
                exit_code: None,
            });
        }
...
impl RoleConfigLocator for AnsibleRuntime {
